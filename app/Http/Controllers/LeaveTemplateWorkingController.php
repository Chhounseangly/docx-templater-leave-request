...
class LeaveTemplateWorkingController extends Controller
{
    ...
    public function generatePdf($id)
    {
            ...
            foreach ($section->getElements() as $element) {
                if ($element instanceof Table) {
                    ...
                    if (count($placeholdersInTables) > 0 && $key) {
                        ...
                        foreach ($placeholdersInTables as $ph) {
                            $value = $this->getNestedValue($sampleData, $mapping[$ph]);
        ...
        return response()->download($pdfPath, '', [], 'inline')->deleteFileAfterSend(true);

        // return response()->download($outputPath, '', [], 'inline')->deleteFileAfterSend(true);
    }
